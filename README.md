...
- 工作组管理，支持新建、删除、重命名、拖动排序、横向调整宽度和工作组全屏。
- 默认分组按“整高半屏 + 两个上下堆叠的四分之一屏”循环向右拼接：1 个分组占满主面板，2 个各占半屏，3 个为左侧整高半屏加右侧两个上下堆叠分组，更多分组以半屏为单位横向分页。
- 桌面端连接侧栏默认收缩，鼠标靠近或键盘焦点进入时自动展开。
- 刷新页面后恢复仍在服务端存活的终端窗口。
- 持久化工作组名称、顺序、横向宽度、终端所属分组和终端窗口高度。
...
点击右上角“主机”按钮，或使用 `Ctrl+Shift+H`（Windows/Linux）/ `Command+Shift+H`（macOS）打开主机管理器。顶部可以切换当前目标分组，也可以直接新增分组；左侧显示识别出的主机，右侧显示当前分组已经打开的终端。切换分组后可以继续打开主机，因此一次打开主机管理器即可向不同分组添加终端。

每台识别主机可以：

- “填入”：把主机信息填入连接表单。
- “打开”：直接打开该主机。
- 勾选主机后使用“打开所选”：选择一个工作组，将自定义选择的多台主机批量打开到该工作组。
...
工作组名称、顺序和手动调整后的横向宽度会保存到浏览器本地，刷新后恢复。主面板不进行纵向滚动；分组内的终端列表和终端历史分别保留纵向滚动。
