...
- 聚焦模式：顶栏一键与工作台模式互相切换（带过渡动画）。顶部标签栏按工作组分区、组色标识；主面板单终端聚焦；左侧栏集成连接表单入口、系统设置入口、快捷操作、同步广播控制与广播历史。在主终端直接打字、回车即同步到同组其他终端（支持全部/已选范围），输出差异的终端标签变色提示；分组图钉、失败重连、广播历史回填一应俱全。
...
- Linux 或 macOS 服务端支持全部功能；Windows 服务端同样支持本机终端（基于 ConPTY，需 Windows 10 1809+），可选 CMD、Windows PowerShell 或 PowerShell 7
...
改动生效方式分三类：`webssh/static/` 下的静态 JS/CSS 修改后刷新页面即可（静态资源带内容版本号，不会读到旧缓存）；`index.html` 模板与后端 `.py` 的修改需要重启服务；开发期可用 `py -3.12 run.py --debug` 启动，模板与代码改动会自动重载。
...
组级重连只处理错误状态且仍持有当前页面重连数据，或无需敏感凭据即可重连的终端。已连接、正在连接及显示“需要重新认证”的终端不会被批量触发。
...
右上角“本机终端”会打开服务端本机 shell。它和 SSH 终端使用相同的窗口、分组、日志、重连和关闭逻辑。Linux 与 macOS 通过 PTY 打开默认 shell；Windows 通过 ConPTY（pywinpty）实现，打开时可按系统安装情况选择 CMD、Windows PowerShell 或 PowerShell 7，默认自动选择可用 shell。Windows shell 不支持 OSC 7 目录跟踪，上传目标会回退到启动目录；终端编码会切换为 UTF-8（chcp 65001）。
