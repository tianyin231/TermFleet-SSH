...
- Treat broadcast commands as potentially sensitive. Keep command history browser-local, bounded, and scoped by group. Built-in candidates must remain read-only diagnostic commands, and neither history nor common candidates may be sent until the user explicitly broadcasts the selected command.
