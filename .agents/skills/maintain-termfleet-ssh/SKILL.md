...
- Keep the server importable and runnable on both POSIX and Windows. Unix-only modules (`pty`, `fcntl`, `termios`) and Unix-only calls (`os.setsid`, `os.killpg`, POSIX signals such as `SIGHUP`) must stay behind `sys.platform` guards, never at module top level unless guarded. Windows servers hide the local-terminal entry through the `window.WSSH_LOCAL_TERMINAL` template flag and reject `/local-terminal` with HTTP 400; preserve that degradation unless a task explicitly replaces it. New platform-dependent features must ship the same three pieces together: backend platform guard, template flag, and frontend hidden/disabled fallback. Every future change must keep both platforms working.
