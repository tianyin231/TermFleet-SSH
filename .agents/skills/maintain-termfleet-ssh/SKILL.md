...
- Keep the server importable and runnable on both POSIX and Windows. Unix-only modules (`pty`, `fcntl`, `termios`) and Unix-only calls (`os.setsid`, `os.killpg`, POSIX signals such as `SIGHUP`) must stay behind `sys.platform` guards, never at module top level unless guarded. The local terminal is PTY-backed on POSIX and ConPTY-backed (`pywinpty`, win32-only dependency) on Windows: the guarded `winpty` import decides `LOCAL_TERMINAL_SUPPORTED`, the `window.WSSH_LOCAL_TERMINAL` template flag gates the UI entry, and `/local-terminal` still answers 400 when the capability is missing — keep that degradation pattern. New platform-dependent features must ship the same three pieces together: backend platform guard, template flag, and frontend hidden/disabled fallback. Every future change must keep both platforms working.
- Keep the focus mode (聚焦模式) and the workspace mode sharing one session core: cards stay rendered into the workspace board even when focus mode hides it, and the focus view only relocates existing cards (never re-creates terminals). After any card relocation, the xterm viewport must be rebuilt (off-screen buffer reflow + fit + refresh) or scrollback breaks. Cluster broadcast only intercepts `term.onData` for the hosted main terminal while sync is on; Enter replicates the typed line to the other targets (the main terminal gets its Enter through its own keystroke flow). History/Tab-completed lines are unknowable from keystrokes, so on Enter they are read back from the hosted terminal's buffer against the per-record learned prompt (learned from ordinary typed Enters, with the OSC 7/133 prompt snapshot as first-fill fallback); when no prompt is known the broadcast degrades to a bare Enter — never guess. The divergence verdict compares each target's log-buffer suffix anchored on the echoed command, requires that anchor (unjudgeable → no verdict, prefer missed over false), and never needs the terminal to be opened; it resolves on the OSC 133 `D;<exit>` mark when the shell hooks emit it (bash/zsh/fish), with the OSC 7 prompt signal and the output-quiet timer as fallbacks, and records the exit code when available. The index page is served with `Cache-Control: no-cache` so browsers always revalidate the HTML that references the hashed static assets. The focus-mode command journal and terminal logs persist in a dual-layer browser store: `sessionStorage` is always written/read (default refresh refill), while `localStorage` mirrors the same keys (`wssh-logs`, `wssh-journal`, `wssh-journal-mains`) only while the `wssh-settings.restartRefill` toggle is on, so a browser restart can refill as well. Closing a terminal removes its records from both layers. A server restart is detected via the per-process `BOOT_ID` (uuid in `handler.py`, exposed as `window.WSSH_BOOT_ID`): the client stores it in `sessionStorage` under `wssh-boot-id`, and on a mismatch with restart-refill off it drops the sessionStorage refill records, because a post-restart page reload is still a "reload" navigation and is otherwise indistinguishable from F5. Persisting these beyond the restart-refill opt-in, or removing the toggle, requires another explicit design decision, and keep click-to-locate tolerant of buffer trims (search upward from the stored row, degrade to a toast rather than a wrong position). Static assets must stay served through `{{ static_url(...) }}` with `static_hash_cache=False` — a process-lifetime cached hash served stale JS across file updates once; do not reintroduce it.
