...
- server-local shell sessions through a Unix PTY (Windows servers hide the local-terminal entry and `/local-terminal` returns HTTP 400);
...
| `/local-terminal` | POST | Create the server user's shell PTY, install supported temporary directory tracking, and return `{id, status, encoding}`; Windows servers reject with HTTP 400 because `pty`/`fcntl`/`termios` are unavailable, and the rendered page hides the toolbar entry via `window.WSSH_LOCAL_TERMINAL` |
...
| `wssh-groups` | group IDs, names, order, `stacked-v1` layout marker, grid spans, manual-size flag, pin state, broadcast scope, line-by-line send toggle/mode/interval, and sanitized pinned-terminal snapshots; older layout records reset to the new default spans on restore |
