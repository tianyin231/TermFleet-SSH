...
- server-local shell sessions through a Unix PTY on POSIX and ConPTY (`pywinpty`) on Windows; `/local-terminal` GET returns `{supported, shells}` (Windows lists pwsh/PowerShell/cmd availability) and POST accepts a `shell` choice (`auto` default), while servers without the capability keep hiding the UI entry and answering HTTP 400;
- a switchable focus mode (聚焦模式) beside the workspace view: top tab strip grouped by color-coded work groups, one hosted terminal in the main panel, and a left sidebar carrying the brand/summary, quick actions (connect/hosts/settings/log/local terminal/new group/theme/language/disconnect-all), the cluster-broadcast control (关/全部/已选, pending line, sync count, line-run progress with stop), divergence indicators on tabs, and broadcast history fill-back. Cards stay rendered into the (hidden) workspace board and are relocated into the main panel; mode choice persists in `wssh-settings.uiMode`. Divergence verdicts resolve on the OSC 133 `D` mark when the shell hooks emit it, falling back to the OSC 7 prompt signal and the output-quiet timer; history/Tab-completed input lines are read back from the hosted terminal's buffer against the per-record learned prompt (bare Enter when no prompt is known). The sidebar carries two tabs: a session-level, group-merged command journal (执行历史, default) populated at broadcast-dispatch/hosted-Enter time with the exactly-known command text (the OSC 133 `C` mark only refines same-command entries within a few seconds; `D` backfills the exit code; 50 entries per terminal; refreshed across page refresh via the dual-layer `wssh-journal`/`wssh-journal-mains` records, and across a browser restart only when the restart-refill setting is on; a server restart (detected via `wssh-boot-id`) drops the refill unless that setting is on), and the existing broadcast-history fill-back tab. The journal renders one row per command: single-terminal entries click through to locate, while same-command entries within a 3 s window (a broadcast round) collapse into one row that expands an elevated, vertically stacked host panel (animated max-height, one host per row). Host dots are gray (pending), green (exit 0), red (exit non-zero), or orange (diverged — only for the broadcast round the current diff verdict belongs to, via `WSSH_CLUSTER.getDiffInfo`). Locating activates that terminal and scrolls to the command, selecting the command-plus-output block (re-located by searching upward from the stored buffer row for the echo; the block ends above the next prompt line; the selection row is derived from `buffer.viewportY` because `scrollToLine` clamps at the bottom; toasts when trimmed away);
...
- an optional per-group line-by-line send mode, switched by a Once/Line by line segmented control in the group tools next to the existing All/Selected scope switch (identical segment styling), with the pacing configuration surfaced on the broadcast bar as a single shared dropdown that swaps between the control-key list and the Interval/Prompt mode list, plus an interval input (mode and interval persisted in the group record): the editor content is split on newlines and each line is sent as its own WebSocket input — either interval-paced or prompt-gated (the client waits for the next OSC 7 `cwd` signal, which the Bash/Zsh/Fish prompt hooks emit on every prompt render, then advances; without a prompt it advances after a fixed 30 s timeout, marks the wait, and continues; unsupported shells degrade to the timeout), blank lines send one bare Enter, standalone control-key lines such as `^c`/`^d`/`^z`/`^l`/`tab`/`enter`/`esc` send only the control sequence without Enter, the send button becomes a live stop/progress control (`停止 i/total`), Esc or button stops with editor content kept, disconnected targets are dropped from the run, the editor clears only on completion, and history records the whole script;
...
- per-group terminal log export honoring the same All/Selected scope as broadcasts and uploads: a download button next to the upload button on the broadcast bar opens a confirmation dialog (same style family as the upload dialog) listing every target with an editable file name (default `<displayName>_<YYYYMMDD-HHmmss>.txt`, local time) and log size, plus an optional save-location picker via the File System Access API (`showDirectoryPicker`, Chrome/Edge only; other browsers fall back to the browser default downloads folder and are told so in the dialog); saving writes plain-text UTF-8 files through the directory handle or blob anchors, appends `.txt` when missing, reports per-run success/failure counts, and is recorded in the operation log; logs are buffered in memory per record (a ring buffer capped at 1 MiB that keeps the newest half on overflow), ANSI OSC/CSI/DCS escapes and control characters are stripped on export, disconnected terminals still export their pre-disconnect history, zero-selection and empty-log cases are blocked with toasts, and the dialog is disabled during a line-by-line run;
- terminal rename, reconnect, move, resize, browser-native single-terminal fullscreen with synchronized Esc/exit restoration, close, and latency display;
...
| `webssh/templates/index.html` | Entire page markup and application CSS, including the focus-mode shell, cluster controls, and the local-terminal shell dialog |
| `webssh/static/js/main.js` | Core client plus workspace view: state, transport, session persistence, i18n, group/card workspace, broadcast and line-send machinery, the focus-mode cluster-broadcast engine (input interception, divergence verdicts, `window.WSSH_CORE`/`WSSH_CLUSTER` surfaces) |
| `webssh/static/js/focus-view.js` | Focus-mode view layer: tab strip, card relocation with xterm viewport rebuild, cluster/diff/history panels, `window.WSSH_FOCUS` |
...
- The vendored xterm.js is a 4.x build whose legacy CompositionHelper reads the hidden textarea on key events (no `input` event listener). During IME composition it only ignores keyCode 229 and 16/17/18, so a Caps Lock (keyCode 20) keydown finalizes the composition early and the following compositionend sends the committed text again — `openSocket` in main.js swallows that keydown via `attachCustomKeyEventHandler` (gated on `event.isComposing`) to keep a single commit; any future IME-input fix should respect this guard.
...
| `/local-terminal` | GET | Return `{supported, shells}`; shells list the Windows choices (pwsh/powershell/cmd) present on the server, `[]` on POSIX |
| `/local-terminal` | POST | POSIX: create the shell from `$SHELL` through a PTY. Windows: spawn the chosen shell (pwsh/PowerShell/cmd, `shell=auto` default) through ConPTY with a pump-thread/socketpair bridge; UTF-8 (`chcp 65001`). Unsupported platforms answer HTTP 400 |
...
2. The backend creates a `Worker`, detects encoding and supported shell type through one existing exec channel, installs non-persistent per-prompt hooks in the interactive PTY when supported (OSC 7 directory tracking plus OSC 133 `A`/`C`/`D;<exit>` semantic marks for bash/zsh/fish), registers the Worker in the canonical `clients[client_ip]` map after asynchronous setup, rechecks `maxconn`, and schedules an owned early-recycle timeout if no WebSocket binds. Batch setup returns one aligned result per requested connection, including per-item failures, and uses `max(options.delay, 30)` seconds for this pending-bind grace.
...
| `wssh-settings` | disconnect confirmation, broadcast Enter, restart refill toggle, terminal font size/height, max terminals, SSH setup concurrency, persistent panel mode, and customizable toolbar shortcut bindings |
...
| `wssh-logs` | per-`persistentId` terminal log tail (200 KB cap, sliced to a whole line) used to refill a terminal after refresh; the same key also exists in `sessionStorage` (always, the default refresh-refill layer) and in `localStorage` (only while the restart-refill setting is on) |
| `wssh-journal` | per-`persistentId` command-journal entries (id, command, time, exitCode, row, batchId, diverged, up to 50 per terminal) plus the journal sequence watermark; dual-layer like `wssh-logs` |
| `wssh-journal-mains` | `batchId -> main persistentId` map for focus-mode cluster batches, restored before cards rebuild; dual-layer like `wssh-logs` |
| `wssh-boot-id` | `sessionStorage` copy of the server's per-process `BOOT_ID` (`window.WSSH_BOOT_ID`); a mismatch on load means the server restarted, so with restart-refill off the sessionStorage refill records (`wssh-logs`/`wssh-journal`/`wssh-journal-mains`) are dropped |
| `wssh-groups` | group IDs, names, order, `stacked-v1` layout marker, grid spans, manual-size flag, `color` (palette index), pin state, broadcast scope, `clusterSend` (focus-mode sync on/off), line-by-line send toggle/mode/interval, and sanitized pinned-terminal snapshots; older layout records reset to the new default spans on restore |
...
- Static assets are referenced through `{{ static_url(...) }}` (content-hashed URLs) with `static_hash_cache=False` in `get_app_settings`; do not reintroduce process-lifetime hash caching (it served stale JS to long-running processes) and do not hardcode `static/js/...` paths. Template and backend `.py` edits still need a server restart; `--debug` enables auto-reload for development.
